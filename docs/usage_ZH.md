...
插件提示段会指导模型按协议执行：评估 Scope 级别 → 上下文扫描与方案综合 → 交互式确认 (Standard/Deep) → 组建团队并创建任务 → 调度器并发分发至空闲成员 → 队长监控与协调 → 多维度代码审查与闭环修复 → 汇报结果并 `agent_teams_delete`。成员之间可以直接互发消息，无需队长中转。成员若在中断、异常结束或进程重启后变成 `idle/ready`，但磁盘上仍持有 `claimed/in_progress` 任务，调度器会撤销旧 capability、生成新 attempt 并重新唤醒同一成员。

### 任务级别与交互式计划确认 (Scope Tiers & Confirmation Gate)

- **Lightweight 极速执行**：针对单文件微调、局部 Bug 修复等轻量级任务，队长在第 1 轮中直接完成建队与任务分发，不暂停等待确认。
- **Standard & Deep 计划确认门禁**：针对多模块功能、架构重构或复杂 DAG 任务，队长必须先完成方案综合，向用户呈现目标、团队角色配置及任务 DAG（含依赖与文件边界），暂停等待用户显式确认后方可建队与分发任务。

### 任务并行执行与多 Worker 扩容

- **并发分发 (Concurrent Dispatch)**：事件驱动调度器会同时为所有当前 `idle` 的成员认领并分发已就绪的任务。当 DAG 中同一 Wave 内存在多个依赖满足的独立任务时，只要团队中有多个 Worker 成员，它们就会真正并发执行。
- **Worker 规模与命名策略**：当任务波次包含多个独立子任务（如为不同模块编写测试套件）时，队长应创建多个专用成员（如 `engineer_1`, `engineer_2`, `engineer_3` 或按领域命名的 `test_engineer`, `ui_engineer`），而不是将所有任务全部分配给单个 `engineer`。建议每波次并发 Worker 数量限制在 3–4 个以内，以防触发 LLM API 速率限制。
- **文件作用域隔离 (Disjoint File Scopes)**：对于并行执行的任务，任务描述中必须明确指定各自互不重叠的目标文件或目录，以杜绝并发写冲突与 Git 合并冲突。
- **质量门禁汇聚**：代码简化 (`simplifier`) 与代码审查 (`reviewer`) 等汇聚门禁任务必须在 dependencies 中列出当前波次所有的并行任务 ID（例如 `dependencies: ["t2", "t3", "t4"]`），等待全部并行任务完成后再统一进行审查。
...
- GUI：独立实例 + ego-browser（详见 `verification-guide_ZH.md`）
