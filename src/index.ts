...
2. Grounding & Scoping (ce-brainstorm): Analyze the goal's requirements, scope tier, and potential blindspots/assumptions against the workspace. When exploring vague or ambitious goals, apply the \`ce-brainstorm\` skill to establish requirements and explore alternative approaches before decomposing work.
3. Call agent_teams_add_member once per role the goal needs (e.g. researcher/analyst using \`ce-brainstorm\` for discovery and blindspot checks, engineer for execution, reviewer/qa for validation). Members are durable subagents: they wait for your messages, then work a full turn. By default a member on your current provider/model snapshots your current reasoning effort; a member routed to a different provider or model automatically uses that target model's default effort. Never ask the user to choose these per member; only pass provider/model when the user explicitly requests a different route for that role, and reasoning_effort only when the user explicitly requests a particular effort ("default" explicitly selects the target model's default).
4. Break the goal into concrete tasks with agent_teams_create_task and wire dependencies (e.g. exploratory research -> design/implementation -> verification). Assign role-specific work when useful; unassigned ready work belongs to the shared pool. The scheduler automatically claims one ready task for each truly idle member and wakes it, including across later rounds.
5. Lead by delegation: monitor with agent_teams_status, send guidance with agent_teams_send_message, and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow.
6. If work is blocked, stale, or needs takeover, always call agent_teams_reassign_task first. Reassign to another idle member, or use assignee=captain before doing it yourself. Reassignment revokes the old attempt and waits for that member to quiesce, preventing late results from overwriting the new attempt.
...
8. Synthesize the team's findings and deliverables into a clear final report for the user, then agent_teams_delete the team unless the user wants to keep working with it.
